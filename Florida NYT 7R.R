...
# raw case counts
NYTplotdaily <-ggplot(data = NYTlineDataDaily, aes(x=days, y=daily, colour = name)) + plotformat(NYTlineDataDaily) + ggtitle("Number of new cases in Florida\n", subtitle = "number of days since 10th case")

# 7 day rolling case count
NYTrolldaily <- ggplot(data = NYTlineDataRolling, aes(x=date, y=movave, color = name)) + plotformat_date(NYTlineDataRolling)+bars()
